...
export const StudioTalkingHead = ({
  ...
  return (
    <AbsoluteFill style={stageStyle}>
      ...
      {talkingHeadVideo ? <PipFrame talkingHeadVideo={talkingHeadVideo} /> : null}
...
const PerspectiveGrid = ({seconds, glow}: {seconds: number; glow: number}) => {
  ...
  const rowStep = 78;
  const rowDrift = (seconds * 38) % rowStep;
  const rows = Array.from({length: 8}, (_, index) => nearY + ((index * rowStep + rowDrift) % planeHeight));
  const verticals = Array.from({length: 17}, (_, index) => index - 8);
  const project = (y: number) => {
    const t = clamp((y - nearY) / planeHeight, 0, 1);
    return {left: -160 + t * 320, right: 1240 - t * 320};
  };
  const mirrorY = (y: number) => canvasHeight - y;
  const nearX = (index: number) => canvasWidth / 2 + index * 132;
  const farX = (index: number) => canvasWidth / 2 + index * 64;
  ...
  return (
    <div
      style={{
        ...captionStyle,
        opacity: visible,
        transform: `translateY(${(1 - visible) * 18}px) scale(${0.98 + visible * 0.02})`,
      }}
    >
        ...
        {chapters.map((chapter, index) => {
          ...
          return (
            <span key={chapter.label} style={chapterGroupStyle}>
              <span style={{...chapterDotStyle, backgroundColor: reached ? colors.accent : colors.topbarSeparator}} />
              <span style={{...chapterLabelStyle, color: reached ? colors.white : colors.topbarMuted, fontWeight: reached ? 600 : 500}}>
                {chapter.label}
              </span>
            </span>
          );
